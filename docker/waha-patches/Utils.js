...
exports.LoadUtils = () => {
    ...
    window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
        ...
        return model;
            ...
            console.error('WHAHA_DEBUG getChatModel fallback for chat', chat.id._serialized, e.message);
            let fallbackName = '';
            try { fallbackName = chat.name || ''; } catch(e2) {}
            try { fallbackName = fallbackName || chat.formattedTitle || ''; } catch(e2) {}
            fallbackName = fallbackName || chat.id?.user || '';
            return {
                id: { _serialized: chat.id._serialized, server: chat.id.server, user: chat.id.user },
                name: fallbackName,
                formattedTitle: fallbackName,
