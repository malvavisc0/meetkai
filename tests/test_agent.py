...
from kai.agent.core import _DEFAULT_SYSTEM_PROMPT, KaiAgent
from kai.agent.goal import GoalManager
from kai.config.settings import Settings


def _mock_llm(reply_content: str = "ok"):
...
class TestKaiAgent:
    ...
    def test_build_messages_extra_context_keeps_base_prompt(self):
        ...
        assert "People in this chat: Alice" in messages[0].content
    ...
    def test_timezone_changes_local_time_in_prompt(self):
        agent = KaiAgent(settings=None, goal_manager=GoalManager())
        agent.set_timezone("America/Santo_Domingo")
        prompt = agent._get_system_prompt()
        # AST (Atlantic Standard Time, UTC-4) is the tzname for Santo Domingo.
    ...
    def test_set_timezone_strips_whitespace(self):
        agent = KaiAgent(settings=None, goal_manager=GoalManager())
        agent.set_timezone("  America/Santo_Domingo  ")
        assert agent._timezone == "America/Santo_Domingo"
    ...
    def test_history_cleared_on_goal_change(self):
        ...
        assert "Current goal: New goal" in messages[0].content
...
class TestKaiAgentChat:
    ...
    async def test_chat_returns_reply(self, settings):
        ...
        reply = await agent.chat("Tell me a joke")
        assert reply == "Why did the chicken?"
    ...
    async def test_chat_per_call_system_prompt(self, settings):
        ...
        await agent.chat("hello", system_prompt="Group: Alice, Bob")
    ...
    async def test_chat_isolates_history_by_conversation(self, settings):
        ...
        await agent.chat("from chat one", conversation_id="chat-1")
        await agent.chat("from chat two", conversation_id="chat-2")
    ...
    async def test_chat_guards_empty_reply(self, settings):
        ...
        reply = await agent.chat("Hello")
        assert reply == "I couldn't generate a response."
    ...
    async def test_chat_persists_history(self, tmp_path, settings):
        ...
        await agent.chat("remember me", conversation_id="chat-1")
        await agent.flush()
    ...
    async def test_chat_persists_goal_with_history(self, tmp_path, settings):
        ...
        assert restored_gm.get_goal().description == "Support users"
...
class TestKaiAgentContext:
    ...
    def test_format_user_message_group_mentions_bot(self):
        ...
        result = agent._format_user_message("hello", ctx)
        assert result == "[Juan Palotes (mentioning you)] hello"

    def test_format_user_message_dm(self):
        agent = KaiAgent(settings=None, goal_manager=GoalManager())
        ctx = MessageContext(
            sender_name="Lucerna",
            sender_id="72013750239365@lid",
            chat_id="72013750239365@lid",
            is_group=False,
        )
    ...
    async def test_silent_stores_nothing_with_context(self, settings):
        ...
        await agent.chat("hello", context=ctx)
        history = agent._get_history()
        assert len(history) == 0
...
class TestKaiAgentToolHistory:
    ...
    async def test_tool_calls_not_in_history(self, settings):
        ...
        reply = await agent.chat("search for test")
    ...
    async def test_tool_error_does_not_leak_into_history(self, settings):
        ...
        reply = await agent.chat("do something")
        assert reply == "I tried but it failed"
    ...
    async def test_multiple_tool_rounds_history_clean(self, settings):
        ...
        reply = await agent.chat("find info about cats")
    ...
    async def test_max_tool_rounds_exceeded(self, settings):
        ...
        reply = await agent.chat("infinite loop")
    ...
    async def test_unknown_tool_error_reported(self, settings):
        ...
        reply = await agent.chat("use the tool")
        assert reply == "Done"
    ...
    async def test_tool_exception_reported(self, settings):
        ...
        reply = await agent.chat("search something")
        assert reply == "Had an issue"
    ...
    async def test_history_preserves_turn_ordering_across_tool_calls(self, settings):
        ...
        await agent.chat("question one")
        await agent.chat("question two")
...
class TestAuditFixes:
    ...
    def test_is_silent_reply_variants(self):
        from kai.agent.core import is_silent_reply

        assert is_silent_reply("<<silent>>") is True
        assert is_silent_reply(" <<silent>> ") is True
        assert is_silent_reply("<<silent>> (low-value)") is True
        assert is_silent_reply("<<silent>> (anything)") is True
        assert is_silent_reply("hello") is False
        assert is_silent_reply("<<silent>> extra text") is False
        # Models sometimes wrap the marker in backticks mirroring prompt
        # code-span formatting; this must still count as silence.
        assert is_silent_reply("`<<silent>>`") is True
        assert is_silent_reply("` <<silent>> `") is True
    ...
    def test_strip_reasoning_channels_removes_channel_blocks(self):
        ...
        assert strip_reasoning_channels(leaked) == "<<silent>>"
    ...
    def test_silent_detected_after_channel_strip(self):
        from kai.agent.core import is_silent_reply, strip_reasoning_channels

        leaked = "<|channel>thought<channel|><<silent>>"
        assert is_silent_reply(strip_reasoning_channels(leaked)) is True
    ...
    async def test_load_goal_does_not_bump_revision(self, tmp_path, settings):
        ...
        assert gm.get_goal().description == "Restored goal"
    ...
    def test_allow_silence_false(self):
        agent = KaiAgent(settings=None, goal_manager=GoalManager())
        agent.set_system_prompt("Base.")
        prompt = agent._get_system_prompt(allow_silence=False)
        assert "Never output <<silent>>" in prompt

    @pytest.mark.asyncio
    async def test_chat_with_reply_style(self, settings):
        agent = KaiAgent(settings=settings, goal_manager=GoalManager())
        agent._llm = _mock_llm("ok")
        await agent.chat("hello", reply_style="Max 1 sentence.")
        call_args = agent._llm.achat_with_tools.call_args_list[0]
        messages = call_args.kwargs["chat_history"]
