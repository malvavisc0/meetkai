"""Tests for the console chat send route's "sent to" confirmation.

``POST /deployments/{id}/chat`` only shows a "sent to X" confirmation when
the bot's own ``/tell`` response confirms it dispatched a real send via a
``send_reply`` action entry (see ``kai.bots.email.Bot.handle_operator``) —
the delivery target is decided by the agent's structured action, not a form
field.
"""
