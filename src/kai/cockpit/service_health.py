...
already surfaced on the deployment page): the LLM provider and the optional
brain services.
...
is a Connections concern, not an infrastructure-health concern, and is
surfaced on the Connections page instead — mixing the two here would make
this module depend on caller-supplied, per-user data instead of being
self-contained like every other check.
...
disabled by config is omitted rather than flagged as down.
"""
...
async def check_service_health() -> list[HealthCheck]:
    ...
    state is likewise out of scope — see the Connections page for that.
    """
    checks: list[HealthCheck] = []
