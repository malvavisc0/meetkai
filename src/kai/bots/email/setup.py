...
timezone) plus email-specific options. ``blacklist`` is a blocklist of
sender addresses to silently ignore — there's no "allow only these senders"
concept here, only a blocklist.
"""
...
class BotConfig(BaseModel):
    ...
    # channel. Sourced from the ``image`` feature flag via config.json
    # (written by the cockpit).
    vision: bool = False
    ...
    # never invent facts, stay concise — so this defaults to a low 0.2.
