...
class TestExtractVideo:
    def test_video_from_url(self):
        msg = {
            ...
            "mimetype": "video/mp4",
            "filename": "clip.mp4",
            "mediaUrl": "http://localhost:3000/api/files/default/clip.mp4",
        }
        result = extract_media(msg)
        assert result is not None
        assert result.type is MediaType.VIDEO
        assert result.url == "http://localhost:3000/api/files/default/clip.mp4"
        assert result.data is None
        assert result.mime_type == "video/mp4"

    def test_video_from_base64(self):
        raw = b"\x00\x00\x00\x1cftypmp42"
        msg = {
            "type": "video",
            "mimetype": "video/mp4",
    ...
    def test_video_from_data_type(self):
        """WAHA _data.type == 'video' with media.url (downloadMedia=true form)."""
        msg = {
            "type": None,
            "_data": {"type": "video"},
            "media": {
                "url": "http://localhost:3000/api/files/default/clip.mp4",
                "mimetype": "video/mp4",
            },
        }
        result = extract_media(msg)
    ...
    def test_video_from_mimetype_only(self):
        """Recognize video from mimetype when type is missing."""
        msg = {
            "mimetype": "video/mp4",
            "mediaUrl": "http://localhost:3000/api/files/default/clip.mp4",
        }
