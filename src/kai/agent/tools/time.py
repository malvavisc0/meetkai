...
def get_current_datetime(timezone: str | None = None) -> dict[str, str]:
    ...
    Args:
        timezone: Optional IANA timezone name (e.g. ``Europe/Berlin``).
            When given, the result includes the current time in that zone.
    """
    ...
    if timezone:
        try:
            ...
            result[timezone] = target.strftime("%Y-%m-%d %H:%M:%S %Z")
        except (ZoneInfoNotFoundError, ValueError):
            logger.warning("unknown timezone requested: %r", timezone)
            result["error"] = f"Unknown timezone: {timezone}"
    return result
